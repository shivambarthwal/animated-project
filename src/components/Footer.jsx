...
function Footer() {
  return (
    <div className=' flex gap-5 w-full h-screen bg-zinc-900 p-20'>
        ...
        <h1 className='text-[9vw] font-semibold uppercase leading-none -mb-10 tracking-tighter'>EYE</h1>
        <h1 className='text-[9vw] font-semibold uppercase leading-none -mb-10 tracking-tighter'>OPENING</h1>
        </div>
        ...
        <div className='w-1/2'>
        <h1 className='text-[7vw] font-semibold uppercase leading-none -mb-10 tracking-tighter'>Presentations</h1>
        <div className='dets mt-10 p-4'>
        {["Instagram", "Facebook", "Twitter", "LinkedIn","SnapChat"].map((item, index) => {
                if(index !== 5) { // Assuming LinkedIn is the last item and its index is 3
                    return (
                        <a className='block text-xl font-light hover:text-2xl hover:text-blue-500' href={`https://${item}.com`} key={index} target="_blank" rel="noopener noreferrer">
                            {item}
                        </a>
                    );
                }
